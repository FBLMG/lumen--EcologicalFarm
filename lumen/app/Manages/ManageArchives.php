 ...
 * Time: 19:23
 */
class ManageArchives extends BaseManage
{
	/**
	 * 表名
	 * @var string
	 */
	static $table = 'db_archives';
	 ...
	 * @return mixed
	 */
	static function getListWithProductTraceability($name = '', $regionId = 0, $countyId = 0, $nextId = 0, $pageSize = 0)
	{
		...
		if ($regionId) {
			...
			$param['region_id'] = intval($regionId);
		}
		if ($countyId) {
			$where[] = '`county_id`=:county_id';
			$param['county_id'] = intval($countyId);
		}
	 ...
	 * @return mixed
	 */
	static function getListWithProduct($userId = 0,$farmId = 0,$name = '',$nextId = 0, $pageSize = 0)
	{
		$order = BaseManage::dbOrderSqlByDesc($nextId, $pageSize, true);
		$where = array();
		$param = array();
